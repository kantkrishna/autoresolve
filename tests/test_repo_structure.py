...
def test_required_root_files_exist(root_dir):
    ...
    for file_name in REQUIRED_FILES:
        ...
        assert file_path.is_file(), f"Repository compliance failure: Missing {file_name}"

def test_required_directories_exist(root_dir):
    """Ensure the Domain-Driven structure remains intact."""
    for dir_name in REQUIRED_DIRECTORIES:
        dir_path = root_dir / dir_name
        assert dir_path.is_dir(), f"Architecture compliance failure: Missing directory {dir_name}"
...
def test_mcp_servers_have_independent_configs(root_dir):
    ...
    for server in mcp_dir.iterdir():
        if server.is_dir():
            # Each MCP server must have its own pyproject.toml to ensure isolated dependencies
            assert (server / "pyproject.toml").is_file(), f"MCP Server {server.name} missing pyproject.toml"