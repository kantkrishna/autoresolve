...
from pydantic import Field, KafkaDsn, PostgresDsn, HttpUrl
...
class AutoResolveConfig(BaseSettings):
    ...
    """
    # API Gateway
    FASTAPI_ENV: str = Field(default="production", description="FastAPI running environment")
    
    # Event Broker
    KAFKA_BOOTSTRAP_SERVERS: str = Field(..., description="Kafka broker address (e.g., localhost:9092)")
    ...
    # AI & Orchestration
    ANTHROPIC_API_KEY: str = Field(..., description="Anthropic API key for Claude 3.5 Sonnet")
    LANGCHAIN_TRACING_V2: str = Field(default="true", description="Enable LangSmith observability")
