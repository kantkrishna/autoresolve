...
class PIIScrubber:
    ...
    def __init__(self) -> None:
        if PRESIDIO_AVAILABLE:
            ...
            self.entities = ["EMAIL_ADDRESS", "PHONE_NUMBER", "CREDIT_CARD", "US_SSN", "IP_ADDRESS"]
            logger.info("Presidio PII Scrubber initialized successfully.")
        else:
            logger.warning("Presidio not installed. PII Scrubber running in regex fallback mode.")
    ...
    def scrub(self, text: str) -> str:
        ...
        try:
            ...
            logger.error(f"Failed to scrub PII from payload. Dropping payload for safety. Error: {e}")
