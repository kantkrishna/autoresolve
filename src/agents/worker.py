...
async def consume_alerts():
    ...
    logger.info(f"🟢 AI Worker successfully connected to Kafka. Listening for alerts on topic '{topic_name}'...")
            ...
            try:
                ...
                if isinstance(payload, dict):
                    thread_id = (
                        ...
                        payload.get("incident_id") or 
                        payload.get("alertname") or 
                        "UNKNOWN"
                    )
                    # Support nested envelope unpacks
                    if thread_id == "UNKNOWN" and isinstance(payload.get("payload"), dict):
                        nested = payload["payload"]
                        thread_id = nested.get("tracking_id") or nested.get("incident_id") or "UNKNOWN"
                ...
                async for event in agent_app.astream(initial_state, config=thread_config):
                    for node_name, _ in event.items():
                        logger.info(f"💾 Node '{node_name}' finished executing state update sequence.")

                logger.info(f"⏸️  GRAPH PAUSED. Next node in queue: ('execution_node',)")
                logger.info(f"👨‍💻 Waiting for Human-in-the-Loop approval for Thread: {thread_id}")

            except Exception as e:
                logger.error(f"❌ Error occurred processing event stream: {str(e)}", exc_info=True)
                continue
    finally:
        await consumer.stop()
